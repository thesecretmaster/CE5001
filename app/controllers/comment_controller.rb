class CommentController < ApplicationController
  ...
  def evaluate
    @post = Post.left_joins(:post_reviews).where.not(post_reviews: {user_id: current_user.id}).or(Post.left_joins(:post_reviews).where(post_reviews: {id: nil})).order(helpers.random_order).first
  ...
  def done
    @post = Post.left_joins(:post_reviews).where.not(post_reviews: {user_id: current_user.id}).or(Post.left_joins(:post_reviews).where(post_reviews: {id: nil})).order(helpers.random_order).first
    if !current_user.admin && @post.present?
      flash[:warning] = "You're not done yet!"
      redirect_to action: :evaluate
    end
  end
